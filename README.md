...
A Stash plugin that imports yt-dlp sidecar metadata directly into the Stash database.
Created by [David Smith](mailto:david@maxprovider.net) · © David Smith 2026
...
```text
yt-dlp / Video Fetcher / MetaFetch
            ↓
      .info.json files
            ↓
   Info JSON Importer
...
✅ Includes a safe Dry Run mode for previewing all changes before importing.
---
...
If you have existing videos without `.info.json` files, use [MetaFetch](https://github.com/maxprovider/MetaFetch) or run yt-dlp with `--write-info-json --skip-download` to generate them retroactively.
